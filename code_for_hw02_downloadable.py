...
def perceptron(data, labels, params={}, hook=None): # It should return a tuple of θ (a ddd by 1 array) and θ0​ (a 1 by 1 array)
...
# labels = np.array([-1, -1, 1, 1]).reshape((1,4))
# data =   np.array([2, 3,  4,  5]).reshape((1,4))
# labels = np.array([1, 1, -1, -1]).reshape((1,4))
data =   [[1,2, 3,  4,  5,6]]
def one_hot(x, k):
    res=np.zeros((k,1))
    res[x-1,0]=1
    return res
data = [one_hot(i,6) for i in data[0]]
data = np.concatenate(data,axis=1)
print(data)
labels = [[1, -1, 1, -1,1,-1]]
labels = np.array(labels)
print(perceptron(data,labels,params={"T":10000}))
theta = perceptron(data,labels,params={"T":10000})[0]
print(theta)
samsung=np.array([1,0,0,0,0,0]).reshape((1,6))
dist=samsung@theta/10**0.5
print(dist)
#Test Cases:
...
for datafn in (super_simple_separable, xor, xor_more, big_higher_dim_separable):
   data, labels = datafn()
   test_linear_classifier(datafn,averaged_perceptron,draw=False)
'''
...
# print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 300)/20)
# print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 300)/20)
# print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 300)/20)
                                                                        # n_train, n_test, it