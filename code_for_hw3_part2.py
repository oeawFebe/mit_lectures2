...
# def perceptron(data, labels,T, params={}, hook=None): # It should return a tuple of θ (a ddd by 1 array) and θ0​ (a 1 by 1 array)
#     # if T not in params, default to 100
#     # T = params.get('T', 100) # meaning n_iter
...
a_feature_func_tup_list=[('cylinders',raw),
    ('displacement',raw),
    ('horsepower',raw),
    ('weight',raw),
    ('acceleration',raw),
    ('origin',raw)]
b_feature_func_tup_list=[('cylinders',one_hot),
    ('displacement',standard),
    ('horsepower',standard),
    ('weight',standard),
    ('acceleration',standard),
    ('origin',one_hot)]
au_data=load_auto_data('auto-mpg.tsv')

# this was almost successful!!!
# for f_set in [a_feature_func_tup_list,b_feature_func_tup_list]:
#     for t in [1,10,50]:
#         for learner in [perceptron,averaged_perceptron]:
#             params={"T":t}
            
#             data,labels=auto_data_and_labels(au_data, f_set)
#             print(data)
#             print(f'result  {learner.__name__} trial_num:{t} is_first_f_set:{f_set==a_feature_func_tup_list}')
#             res=xval_learning_alg(learner,data,labels,10,params)
#             print(res)
#             print(learner(data,labels,params))

dic_=load_review_data('reviews.tsv')
labels=[]
texts_=[]
for d_ in dic_:
    lab_=d_['sentiment']
    labels.append(lab_)
    texts_.append(d_['text'])
labels=np.array(labels).reshape((1,len(labels)))
bags=bag_of_words(texts_)
data=extract_bow_feature_vectors(texts_,bags)
...
#     for learner in [perceptron,averaged_perceptron]:
#         params={"T":t}
#         print(f'result  {learner.__name__} trial_num:{t}')
#         res=xval_learning_alg(learner,data,labels,10,params)
#         print(res)
theta,theta_0 = averaged_perceptron(data,labels,params={"T":10})
list_of_impact=theta.flatten().tolist()
list_pos_impact=[(pos,impact) for pos, impact in enumerate(list_of_impact)]
# list_pos_impact = sorted(list_pos_impact,key=lambda x:abs(x[1]),reverse=True)[:10]
# list_pos_impact = sorted(list_pos_impact,key=lambda x:x[1],reverse=True)[:10]
list_pos_impact = sorted(list_pos_impact,key=lambda x:x[1],reverse=False)[:10]
print(list_pos_impact)
to_get = [x[0] for x in list_pos_impact]
wordss=[x for x in bags.keys()]
ans=[]
for pos in to_get:
    print(wordss[pos])
    ans.append("'"+wordss[pos]+"'")
ans_str=','.join(ans)
print(ans_str)

# this is really interesting 
# if abs(x[1]) is the lambda sort key: then 
